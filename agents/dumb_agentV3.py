...
while not rospy.core.is_shutdown():
    ...
    if (sensorsG.time - day_clock > 0 and sensorsG.time - day_clock < 20):
       if(pump_on == False and days_passed > 5 and sensorsG.water_level > 20):
           print("pump turned on at %.1f" % (sensorsG.time - day_clock))
           wpump_pub.publish(True)
           pump_on = True
    elif (sensorsG.time - day_clock > 20):
       if(pump_on == True and days_passed > 5):
