...
while not rospy.core.is_shutdown():
    ...
    if (sensorsG.time - last_ping) >= 180: 
        ping()

    if (sensorsG.time - day_clock) >= 86400:
    	print("24hr_clock reset")
    	clock()
    
    #Stuff that happens during the night 
    if (((sensorsG.time - day_clock < 28800) or (sensorsG.time - day_clock > 61200))):
       if(lights_on == True):
           print("turned lights lower at %1.f " % (sensorsG.time - day_clock))
           led_pub.publish(0)
           lights_on = False
    #Stuff that happens during the day 
    elif ((sensorsG.time - day_clock > 28800) and (sensorsG.time - day_clock < 61200)):
       if(lights_on == False):
           print("turned lights higher at %.1f" % (sensorsG.time - day_clock))
           led_pub.publish(245)
           lights_on = True
    #Stuff that happens every 15 mins
    if (((sensorsG.time - day_clock) % 900) > 0 and ((sensorsG.time - day_clock) % 900) < 240):
       if(fans_on == False):
           print("fan turned on at %.1f" % (sensorsG.time - day_clock))
           fan_pub.publish(True)
           fans_on = True
    elif(((sensorsG.time - day_clock) % 900) > 240):
       if(fans_on == True):
           print("fan turned off at %.1f" % (sensorsG.time - day_clock))
           fan_pub.publish(False)
           fans_on = False
    #Stuff that happens every day
    if (sensorsG.time - day_clock > 0 and sensorsG.time - day_clock < 45):
       if(pump_on == False and days_passed > 2 and sensorsG.water_level > 45):
           print("pump turned on at %.1f" % (sensorsG.time - day_clock))
           wpump_pub.publish(True)
           pump_on = True
    elif (sensorsG.time - day_clock > 45):
       if(pump_on == True and days_passed > 2):
